...
*
	...
	@Override public void daxpy(double alpha, DoubleMatrix1D x, DoubleMatrix1D y){
		if ( x.length != y.length)
			throw new IllegalArgumentException("Vectors are of different dimension.");
		int i = 0;
		for(; i < x.length; i++)
	...
	@Override public void daxpy(double alpha, DoubleMatrix2D A, DoubleMatrix2D B){
		...
		int j = 0;
		final int unroll = 6;
		int width = A.rows() - A.rows()%unroll;
		int height = A.cols() - A.cols()%unroll;
		
		for (; i < width; i+=unroll)
		{
			for(; j < height; j+= unroll)
			{
				B.set(i,j, B.get(i,j) + A.get(i,j)*alpha);
				B.set(i+1,j+1, B.get(i+1,j+1) + A.get(i+1,j+1)*alpha);
				B.set(i+2,j+2, B.get(i+2,j+2) + A.get(i+2,j+2)*alpha);
				B.set(i+3,j+3, B.get(i+3,j+3) + A.get(i+3,j+3)*alpha);
				B.set(i+4,j+4, B.get(i+4,j+4) + A.get(i+4,j+4)*alpha);
				B.set(i+5,j+5, B.get(i+5,j+5) + A.get(i+5,j+5)*alpha);
...
*
	...
	@Override public void dcopy(DoubleMatrix1D x, DoubleMatrix1D y){
		...
		DoubleMatrix1D.copy(x,0,y,0,x.length);
	}
	/**
	...
	@Override public int idamax(DoubleMatrix1D x)
	{
		...
		if (1==x.length)
			return 0;
		double max = MathUtils.abs(x.get(0));
		int max_idx = 0; 
		for (int i = 1; i < x.length; i++)
		{
			if(MathUtils.abs(x.get(i)) > max)
			{
				max = MathUtils.abs(x.get(i));
				max_idx = i;
			}
		}
		return max_idx; 
	...
	public static void main(String[] args)
	{
		...
		new ReferenceBLAS().dgemv(false,1, test2, test, 1, test3);
